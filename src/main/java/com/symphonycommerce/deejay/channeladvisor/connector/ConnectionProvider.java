...
public class ConnectionProvider {
  ...
  public ConnectionProvider(ChannelAdvisorLiveConfig liveConfig) {
    ...
    liveConfig.getBrandToRefreshToken().forEach((brand, token) -> {
      brandToAuthToken.put(brand, new OAuthToken(token));
    });

    this.client = ClientBuilder
        .newBuilder()
        .property(LoggingFeature.LOGGING_FEATURE_VERBOSITY_CLIENT, LoggingFeature
  ...
  /**
   * Will return a WebTarget that can be used to make a HTTP request against CA. It will
   * perform the necessary OAUTH steps like keeping track of token expiry and fetching
   * a new token.
   */
  public WebTarget getAuthenticatedClient(String brand) {
    ...
    if (auth.getExpirationTime().isBeforeNow()) {
      ...
      DateTime newTime = DateTime.now().plusSeconds(newAuthTokens.getExpiresInSeconds())
  ...
  public Oauth2Response getNewAccessToken(OAuthToken auth) {
    ...
    HttpAuthenticationFeature feature = HttpAuthenticationFeature
        .basic(config.getAppId(), config.getSharedSecret());
    ...
    return client.target("https://api.channeladvisor.com")
        .path("/oauth2/token")
        .request()
