...
public class SkubanaConnection implements ConfigurableConnection, EcommerceConnection {
  ...
  /**
   * The distribution center IDs are unique to an account and therefore we need to keep
   * track of each one. They are also difficult to extract unless you are technical, which
   * is why I decided to grab it using an API. This keeps the setup for a new client
   * to a minimum mod in the DB rather than extensive configuration changes.
   */
  private LoadingCache<String, Integer> distributionCenterIds = CacheBuilder.newBuilder()
      .maximumSize(20)
      .build(
  ...
  @Override
  public List<ProductEntity> getAllProducts(String brand) throws ProcessingException,
  ...
  DistributionCenter findSymphonyDc(String brand) {
    return provider.getAuthenticatedClient(brand)
  ...
  @Override
  public ApiListWrapper<? extends ProductEntity> getPagedProducts(
      String brand, Integer page) {
   ...
   */
  public ApiListWrapper<? extends OrderEntity> getPagedNewOrders(String brand, Integer
      page) {
    Integer dcId = distributionCenterIds.getUnchecked(brand);
    Preconditions.checkNotNull(dcId);

    ApiListWrapper<Order> result = provider.getAuthenticatedClient(brand)
        .path("/v1/orders")
        .queryParam("$expand", "Items($expand=Adjustments),Fulfillments($expand=Items)")
        .queryParam("exported", false)
        .queryParam("$skip", page)
        .queryParam("$filter", "PaymentStatus eq 'Cleared' and Fulfillments/Any (c: "
            + "c/DistributionCenterID eq " + dcId + ") and ShippingStatus ne 'Shipped'"
  ...
  public ApiListWrapper<? extends OrderEntity> getPagedRecentOrders(String brand, Integer page) {
    return provider.getAuthenticatedClient(brand)
        .path("/v1/orders")
        .queryParam("$expand", "Items($expand=Adjustments),Fulfillments($expand=Items)")
        .queryParam("$filter", "ShippingStatus ne 'Shipped' and ShippingStatus ne "
            + "'Canceled'")
  ...
  public void acknowledgeOrder(String brand, String marketplace, String id) {
    errorsToExceptions(provider.getAuthenticatedClient(brand)
   ...
   */
  @Override
  public void shipOrderFullyOrPartialy(String brand, String marketplace, String orderId,
   ...
   */
  @Override
  public void cancelItems(String brand, String marketplace, String orderId, String orderItemId, int
      qtyToCancel) {
    OrderItemAdjustment adjustment = OrderItemAdjustment.makeCancellation(qtyToCancel, "Symphony-"
        + orderItemId);
    errorsToExceptions(provider.getAuthenticatedClient(brand)
  ...
  /**
   * Updates the amount of InStock inventory available on Channel Advisor for a given CA
   * product ID.
   */
  @Override
  ...
  /**
   * A helper method that will take a Jersey response and throw an exception if
   * the response is not successful (200).
   */
   ...
   */
  public List<?> collectPages(
      BiFunction<String, Integer, ApiListWrapper<?>> fn,
      String
          brand) {
