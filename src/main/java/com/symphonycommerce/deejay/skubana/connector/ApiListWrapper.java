...
public class ApiListWrapper<T extends Entity> implements com.symphonycommerce.deejay.ecommerce
    .ApiListWrapper<T> {
  ...
  public Optional<Integer> getSkipAmount() {
    Optional<NameValuePair> pair = URLEncodedUtils.parse(nextLink, Charset.defaultCharset())
        .stream()
        .filter(x -> x.getName().equals("$skip"))
        .findFirst();

    return pair.isPresent() ? Optional.of(Integer.parseInt(pair.get().getValue())) : Optional
        .empty();
  }
}
